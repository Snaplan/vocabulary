...
def command_for_see():
    ...
    see_scroll_bar = ttk.Scrollbar(see_frame, orient=VERTICAL, command=see_canvas.yview)
    see_canvas.configure(yscrollcommand=see_scroll_bar.set)
    see_canvas.bind('<Configure>', lambda e: see_canvas.configure(scrollregion=see_canvas.bbox("all")))
    ...
    see_win.geometry(f"{int(see_win.winfo_screenwidth() * (1 / 3))}x{int(see_win.winfo_screenheight() * 0.5)}")
    ...
    def show(is_sort):
        ...
        if is_sort:
            ...
            for i, (word, mean) in enumerate(sort_voca):
                word = Label(second_see_frame, text=word, width=see_width, height=see_height)
                mean = Label(second_see_frame, text=mean, width=see_width, height=see_height)
    ...
    checkVar = IntVar()
    ascending = Checkbutton(see_win, text="Ascending", variable=checkVar, command=toggle)
    ascending.select()
    ascending.pack()
    see_win.mainloop()
...
def command_for_modify():
            ...
            modify_combo = ttk.Combobox(modify_win, values=modify_list, width=86, height=5, font="Helvetica 25 bold")
            ...
            if ts_btn_select == True:
                ...
                translate_btn.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.03)
            modify_win.mainloop()
...
def command_for_setting():
    ...
    setting_win.geometry(
        f"{int(setting_win.winfo_screenwidth() * (1.5 / 3))}x{int(setting_win.winfo_screenheight() * 0.55)}")
    english_btn = Button(setting_win, text=english_text, command=click_english_btn, width=20, height=4, font="Helvetica 25 bold")
    e_lb = Label(setting_win, width=20, height=10, bg="#50BCDF")
    korean_btn = Button(setting_win, text=korean_text, command=click_korean_btn, width=20, height=4, font="Helvetica 25 bold")
    ...
    tsvalue.set(True)
    CheckVar = IntVar()
    ts_combo = ttk.Combobox(setting_win, values=ts_combo_list, width=20, height=2, font="Helvetica 25 bold")
    ...
    dest_btn = Button(setting_win, width=20, height=4, command=command_for_dest_btn, text=dest_text, font="Helvetica 10 bold")
    ...
    def ts_btn():
        ...
        if tsvalue.get() == True:
            ...
            tsvalue.set(True)
            ts_btn_select = False
    translate_button = Checkbutton(setting_win, text=translate_text, variable=CheckVar, width=20, height=4, font="Helvetica 25 bold", command=ts_btn)
...
def command_for_delete():
        ...
        delete_combo = ttk.Combobox(delete_win, values=delete_list, width=96, height=5, font="Helvetica 25 bold")
...
def command_for_add():
    ...
    def command_for_add_btn():
        ...
        _mean = mean_entry.get()
        cvoca.add(_word, _mean)
        success_label = Label(add_win,text="\"{}\" {} \"{}\" {}".format(_word, add_win_and, _mean, add_win_success_text),font="Helvetica 25 bold")
        success_label.place(relx=0.01, rely=0.945)
    ...
    add_win.geometry(f"{int(add_win.winfo_screenwidth() * (2.5 / 3))}x{int(add_win.winfo_screenheight() * 0.8)}")
    ...
    add_btn = Button(add_win, command=command_for_add_btn, text=add_win_add_btn_text, font="Helvetica 50 bold")
    close_btn = Button(add_win, command=add_win_quit, text=add_win_close_btn_text, font="Helvetica 50 bold")
    if ts_btn_select == True:
        translate_btn = Button(add_win, command=translate, text=add_win_trans_btn_text, font="Helvetica 20 bold")
        translate_btn.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.03)
...
def main():
    ...
    see_btn = Button(win, text=win_see_btn_text, command=command_for_see, width=40, height=5)
    add_btn = Button(win, text=win_add_btn_text, command=command_for_add, width=40, height=5)
    modify_btn = Button(win, text=win_modify_btn_text, command=command_for_modify, width=40, height=5)
    delete_btn = Button(win, text=win_delete_btn_text, command=command_for_delete, width=40, height=5)
    setting_btn = Button(win, text=win_setting_btn_text, command=command_for_setting, width=40, height=5)
    quit_btn = Button(win, text=win_close_btn_text, command=win.destroy, width=35, height=5)
    see_btn.pack()
    add_btn.pack()
    modify_btn.pack()
    delete_btn.pack()
