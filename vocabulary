...
class Vocabulary:
    def __init__(self, language):
        if language.lower() == 'korean' or language.lower() == 'english':
            if language.lower() == 'korean':
                print(f'당신의 언어는 \'{language}\' 입니다')
            else:
                print(f'You are languege is \'{language}\'')
        else:
            while True:
                language = input('언어를 다시 입력하세요\n')
                if language.lower() == 'korean' or language.lower() == 'english':
                    break
    ...
    def modify(self, word, mean):
        ...
        if word in self.mjvoca:
            ...
            if self.language.lower() == 'korean':
                ...
                print(
                    f"The corresponding \'{word}\' word is not registered in the vocabulary. So progran couldn't modify that \'{word}\' word")
